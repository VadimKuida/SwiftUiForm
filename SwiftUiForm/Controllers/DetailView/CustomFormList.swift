//
//  CustomFormList.swift
//  SwiftUiForm
//
//  Created by Вадим Куйда on 26.05.2020.
//  Copyright © 2020 Вадим Куйда. All rights reserved.
//

import SwiftUI

struct CustomFormList: View {
    ...
    var body: some View {
                ...
                ForEach(fetch.todos, id: \.self)
                { todo in
         ZStack(alignment: .trailing){

                         VStack(alignment: .leading){

                         Text(todo.name).font(Font.custom("Circe", size: 10))
                             .foregroundColor(Color.init(#colorLiteral(red: 0.7843137255, green: 0.7803921569, blue: 0.8, alpha: 1)))
                             .padding([.leading, .bottom])
                             .lineLimit(1)
                          Text(todo.question).font(Font.custom("Circe", size: 15))
                             .foregroundColor(Color.init(#colorLiteral(red: 1, green: 1, blue: 1, alpha: 1)))
                             .padding([.leading, .bottom])
                             .lineLimit(1)
                         Text(todo.description).font(Font.custom("Circe", size: 10))
                             .foregroundColor(Color.init(#colorLiteral(red: 0.7843137255, green: 0.7803921569, blue: 0.8, alpha: 1)))
                             .padding([.leading, .bottom])
                             .lineLimit(2)
                         ...
                         Image(Glossary.isTag(numTag: todo.imageNameId) )
                             .resizable()
...
struct CustomFormList_Previews: PreviewProvider {
